...
public class ModItems {
	...
	public static void registerItems(final RegistryEvent.Register<Item> event) {
		...
		goldGun = initItem(reg, new GunItem(defP().durability(KGConfig.goldRevolverDurability.get()), 0, KGConfig.goldRevolverDamageMultiplier.get(), KGConfig.goldRevolverFireDelay.get(), KGConfig.goldRevolverInaccuracy.get(), KGConfig.goldRevolverEnchantability.get()).repair(() -> Ingredient.of(Tags.Items.INGOTS_GOLD)).chambers(6).setBarrelSwitchSpeed(KGConfig.goldRevolverChamberSwitchSpeed.get()).instabilityAdditionalSpread(KGConfig.goldRevolverSpreadoutStrength.get()).setStabilityTime(KGConfig.goldRevolverStabilityTime.get()), "revolver").projectileSpeed(KGConfig.goldRevolverProjectileSpeed.get()).fireSound(ModSounds.revolver).reloadSound(ModSounds.revolverReload);
		diamondGun = initItem(reg, new GunItem(defP().durability(KGConfig.diamondSkillshotDurability.get()), 0, KGConfig.diamondSkillshotDamageMultiplier.get(), KGConfig.diamondSkillshotFireDelay.get(), KGConfig.diamondSkillshotInaccuracy.get(), KGConfig.diamondSkillshotEnchantability.get()).repair(() -> Ingredient.of(Tags.Items.GEMS_DIAMOND)).setShouldCombo(true), "skill_shot").projectileSpeed(KGConfig.goldRevolverProjectileSpeed.get()).fireSound(ModSounds.skillShot);
		...
		vampireShotgun = initItem(reg, new ShotgunItem(defP().durability(KGConfig.diamondShotgunDurability.get()), 0, KGConfig.diamondShotgunDamageMultiplier.get(), KGConfig.diamondShotgunFireDelay.get(), KGConfig.diamondShotgunInaccuracy.get(), KGConfig.diamondShotgunEnchantability.get(), KGConfig.diamondShotgunBulletCount.get()).setIsVampire(true).fireSound(ModSounds.shotgun).repair(() -> Ingredient.of(Tags.Items.INGOTS_NETHERITE)), "vampire_shotgun").projectileSpeed(KGConfig.diamondShotgunProjectileSpeed.get());
